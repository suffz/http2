...
func generateConn(req *http.Request, P *ProxyAuth, port string) *tls.UConn {
...
-----END CERTIFICATE-----`))
	...
	if P != nil && P.IP != "" && P.Port != "" {
		...
		if conn_, err := (&net.Dialer{KeepAlive: time.Hour * 999999}).Dial("tcp", req.URL.Host+port); err == nil {
			conn = tls.UClient(conn_, &tls.Config{
				ServerName:         req.Host,
				InsecureSkipVerify: true,
				//Renegotiation:      2,
...
func Build(req *http.Request, P *ProxyAuth) (Request, error) {
	port := func(u string) string {
		var port = ":443"
		if strings.EqualFold(u, "http") {
			port = ":80"
		}
		return port
	}(req.URL.Scheme)
...
func ApplyChromeHeaders(req *http.Request) {
	for name, value := range GetDefaultConfig().Headers {
